...
import { RouteStatus, isValidStatusTransition } from '../enums/route-status';
...
export class RouteValidatorService {
  ...
  async validateDriverAssignment(
    ...
    const whereConditions: FindOptionsWhere<Route> = {
      driver_id: driverId,
  ...
  async validateUniqueRouteCode(routeCode: string, excludeRouteId?: string): Promise<void> {
    const whereConditions: FindOptionsWhere<Route> = { route_code: routeCode };
    if (excludeRouteId) {
      whereConditions.id = Not(excludeRouteId);
    }
  ...
  validateRouteDates(plannedDate: Date, plannedStartTime?: string, plannedEndTime?: string): void {
    ...
    if (plannedStartTime && plannedEndTime) {
      ...
      if (endMinutes <= startMinutes) {
        throw new BadRequestException('Horário de término deve ser posterior ao horário de início');
      }
    }
  }
  /**
