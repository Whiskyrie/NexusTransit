...
# Comando CLEAN
cmd_clean() {
    echo -e "${YELLOW}⚠️  Limpando ambiente Docker...${NC}"
    
    if [[ "$1" == "--all" ]]; then
        echo -e "${RED}🗑️  Removendo TUDO (containers, volumes, imagens)...${NC}"
...
# Comando TOOLS
cmd_tools() {
    echo -e "${BLUE}🛠️  Iniciando ferramentas de administração...${NC}"
